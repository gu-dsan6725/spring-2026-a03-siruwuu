...
# Optional: load .env locally (recommended for notebooks/scripts)
try:
...
def retrieve_q4_context(repo_dir: str, max_snippets: int = 10) -> str:
    ...
    sections.append(
        ...
        + run_cmd(
            ...
            "-type f \\( -iname '*auth*' -o -iname '*oauth*' -o -iname '*token*' -o -iname '*jwt*' -o -iname '*security*' "
            "-o -iname '*middleware*' -o -iname '*permission*' -o -iname '*scope*' -o -iname '*role*' \\) -print | head -n 200",
            cwd=cwd,
        ).strip()
        + "\n"
    )
    ...
    searches = [
        ("FastAPI security wiring", r"(OAuth2|HTTPBearer|Bearer|Security\(|Depends\(|fastapi\.security|Authorization:|X-API-Key)"),
        ("Token/JWT validation", r"(jwt|JWKS|JWK|decode\(|verify|signature|bearer|token_url|tokenUrl|issuer|audience|kid)"),
        ("Authorization / scopes / roles", r"(scope|scopes|role|roles|permission|permissions|authorize|authorized|RBAC|ACL)"),
    ]

    # Gather hits and expand with code snippets (line-numbered)
    expanded_snippets = []
    seen_files = set()
...
"""
    ...
    # Pick any model you have enabled in Groq
    # Common choices: "llama-3.1-70b-versatile", "llama-3.1-8b-instant"
...
if __name__ == "__main__":
    ...
    context = retrieve_q4_context(REPO_DIR, max_snippets=12)
    ...
    print(f"Saved Q4 answer to {RESULTS_TXT}")